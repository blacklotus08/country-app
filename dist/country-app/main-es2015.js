...
let AddCountryComponent = class AddCountryComponent {
    constructor(countriesService, spinner, route, router, location, fb, toastr) {
...
class ValidateIdNotTaken {
    static createValidator(countryService) {
        return (control) => {
            return countryService.CheckCode(control.value).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_1__["debounceTime"])(1000)).map(res => {
                this.countries = res;
                this.countries = this.countries.filter((country) => (country.country_code === control.value || country.country_code.toLocaleLowerCase === control.value.toLowerCase()));
...
let CountriesService = class CountriesService {
    ...
    getCountry() {
        return this.httpService.get(this.baseUrl, this.httpOptions).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["tap"])(data => console.log('All: ' + JSON.stringify(data))), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["catchError"])(this.handleError));
    ...
    getCountryById(id) {
        ...
        return this.httpService.get(this.baseUrl + '/' + id, this.httpOptions)
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["tap"])(data => console.log('getCountry: ' + JSON.stringify(data))), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["catchError"])(this.handleError));
    }
    ...
    CheckCode(id) {
        return this.httpService.get(this.baseUrl, this.httpOptions).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["tap"])(data => console.log('Record Check Code:' + JSON.stringify(data))), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["catchError"])(this.handleError));
    }
    handleError(err) {
...
let CountryDetailService = class CountryDetailService {
    constructor(httpService) {
        this.httpService = httpService;
...
let CountryDetailComponent = class CountryDetailComponent {
    constructor(acttivatedRoute, countryDetailService, spinner, location) {
...
let CountryEditGuard = class CountryEditGuard {
...
let CountryComponent = class CountryComponent {
    constructor(countriesService, spinner, toastr, router) {
        this.countriesService = countriesService;
        this.spinner = spinner;
        this.toastr = toastr;
        this.router = router;
    ...
    performFilter(filterBy) {
        ...
        if (this._regionFilter == '') {
            return this.countries.filter((country) => (country.country_code.toLocaleLowerCase().indexOf(filterBy) !== -1 || country.country_name.toLocaleLowerCase().indexOf(filterBy) !== -1));
        }
        else {
            return this.countries.filter((country) => (country.country_code.toLocaleLowerCase().indexOf(filterBy) !== -1 || country.country_name.toLocaleLowerCase().indexOf(filterBy) !== -1)
    ...
    performFilterbyRegion(filterBy) {
        console.log(filterBy + 'Filter by Region');
        return this.countries.filter((country) => country.country_region.indexOf(filterBy) !== -1);
    }
...
let EditCountryComponent = class EditCountryComponent {
    constructor(acttivatedRoute, countriesService, spinner, route, router, location, fb, toastr) {
        this.acttivatedRoute = acttivatedRoute;
        this.countriesService = countriesService;
        this.spinner = spinner;
        this.route = route;
    ...
    getCountry(id) {
        this.countriesService.getCountryById(id)
            .subscribe((country) => this.displayCountry(country), (error) => this.errorMessage = error);
    }
